...
'''
    ...
    - types - optional (if not used, will return all monitors types (HTTP, keyword, ping..) in an account. 
                 Else, it is possible to define any number of monitor types like: types=1-3-4)
    - statuses - optional (if not used, will return all monitors statuses (up, down, paused) in an account. 
                 Else, it is possible to define any number of monitor statuses like: statuses=2-9)
    - custom_uptime_ratios - optional (defines the number of days to calculate the uptime ratio(s) for. 
                 Ex: custom_uptime_ratios=7-30-45 to get the uptime ratios for those periods)
    - custom_uptime_ranges - optional (defines the ranges to calculate the uptime ratio(s) for. 
                 Ex: custom_uptime_ranges=1465440758_1466304758 to get the uptime ratios for those periods. 
                 It is possible to send multiple ranges like 1465440758_1466304758-1434682358_1434855158)
    ...
    - all_time_uptime_durations - optional (returns the "all time durations of up-down-paused events". 
                 It will slow down the response a bit and, if not really necessary, suggest not using it. Default is 0)
    - logs - optional (defines if the logs of each monitor will be returned. 
                 Should be set to 1 for getting the logs. Default is 0)
    - logs_start_date - optional and works only for the Pro Plan as 24 hour+ logs are kept only in the Pro Plan 
                 (starting date of the response times, formatted as Unix time and must be used with 
                 response_times_end_date) (can only be used if monitors parameter is used with a single id and 
                 response_times_end_date - response_times_start_date can't be more than 7 days)
    - logs_end_date - optional and works only for the Pro Plan as 24 hour+ logs are kept only in the Pro Plan 
                 (ending date of the response times, formatted as Unix time and must be used with 
                 response_times_start_date) (can only be used if monitors parameter is used with a single id and 
                 response_times_end_date - response_times_start_date can't be more than 7 days)
    - logs_limit - optional (the number of logs to be returned (descending order). If empty, all logs are returned.
    - response_times - optional (defines if the response time data of each monitor will be returned. 
    ...
    - response_times_start_date - optional (starting date of the logs, formatted as Unix time and must be used 
                 with logs_end_date) (can only be used if monitors parameter is used with a single id)
    - response_times_end_date - optional (ending date of the logs, formatted as Unix time and must be used with 
                 logs_start_date) (can only be used if monitors parameter is used with a single id)
    - alert_contacts - optional (defines if the alert contacts set for the monitor to be returned. Default is 0)
    - mwindows - optional (the maintenance windows for the monitor which can be mentioned with their 
                 IDs like 345-2986-71)
    - custom_http_headers - optional (defines if the custom HTTP headers of each monitor will be returned. 
    ...
    - limit - optional (used for pagination. Defines the max number of records to return for the response. 
